...
/* ---------- Fine art: semicircular art wheel (Personal Studio) ----------
   A shallow arc of paintings that continuously drifts, like a slow
   carousel — never spins like a wheel, never pauses on hover. */
(function setupArtWheel() {
  ...
  function layout() {
    ...
    pieces.forEach((piece) => {
      ...
      piece.style.transform =
        `translate(-50%, -50%) translate(${x - centerX}px, ${y - centerY + h * 0.5}px) rotate(${angle * 0.15}deg) scale(${scale})`;
  ...
  if (!prefersReducedMotion) {
    const SPEED = 3.0; // degrees per second
