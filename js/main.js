...
if (heroClipErase && heroClip && heroClipFinal) {
  if (prefersReducedMotion) {
    ...
    // Delays are measured from the moment the intro actually starts — i.e.
    // once the erase clip's first frame ("Hi, I'm Elena") is decodable — so
    // the greeting never pops in blank on a slow load.
    ...
    const startHeroIntro = () => {
      // Greeting pops in first (as one hand-lettered unit), then the tagline
      // words stagger in just behind it. Both happen UP FRONT so the
      // multidisciplinary intro is readable right away — it's no longer
      // gated behind the full erase + "Nice to meet ya" write cycle.
      revealHeroWriting();
      setTimeout(revealHeroTagline, TAGLINE_AFTER_GREETING_MS);
      // Hold on the greeting for a beat, then start erasing it. The erase
      // plays from the SAME element that just popped in, so there's no
      // swap/seam between the greeting and the video that erases it.
      setTimeout(() => {
        heroClipErase.play().catch(() => {});
      }, GREETING_HOLD_MS);
    };

    // readyState >= 2 (HAVE_CURRENT_DATA) means frame 0 is already available,
    // so start now; otherwise wait for it so the pop isn't a blank frame.
    if (heroClipErase.readyState >= 2) {
      startHeroIntro();
    } else {
      heroClipErase.addEventListener('loadeddata', startHeroIntro, { once: true });
    }
