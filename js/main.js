...
if (heroClipErase && heroClip && heroClipFinal) {
  if (prefersReducedMotion) {
    ...
    revealHeroTagline(); // no video sequence to wait for
  } else {
    ...
    // Tagline (secondary text) waits for the name-writing sequence to
    // actually finish before it starts popping in — previously it began
    // almost immediately on page load and was fully done before "Hi, I'm
    // Elena... Nice to meet ya" had even started, which read as backwards.
    heroClip.addEventListener('ended', revealHeroTagline);
