...
(function setupHeroSketchReveal() {
  ...
  if (prefersReducedMotion) {
    colorLayer.classList.add('is-revealed'); // CSS already shows it instantly with no transition anyway — this just keeps state consistent
    return;
  }

  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      // triggers once the hero has been scrolled mostly past — matches
      // "the second they scroll [away]" per Elena's spec
      if (!entry.isIntersecting || entry.intersectionRatio < 0.4) {
        colorLayer.classList.add('is-revealed');
        observer.unobserve(heroSection); // one-time — never reverses on scroll back up
      }
    });
  }, { threshold: [0, 0.4] });
