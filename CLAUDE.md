...
**Current build state (2026-08-04, session 5):** ALL sections are now **built** — Hero, **About** (inverted plum, 2-col), **Achievements** (illustrated STICKER SHEET), Fine Art grid, **Design for Learning** (rebuilt to mockup — two cards + vertical "velt" + rigged Stellina character, see decision below), **Presentation** (pitch decks; logo belt removed), Stellina scene, Contact. Context: the site doubles as a **portfolio link to send a creative director (meeting 2026-08-05)**; near-term priority is Elena's remaining COPY + the deploy, not new sections.
...
**Cache-busting / stale serving.** css/js are linked as `css/style.css?v=N` + `js/main.js?v=N`. **Bump N on every css/js change** or the preview — and Elena's deploy — serves stale files. This bit us repeatedly (missing flowers, un-run JS). Current version: **v=64**.
...
**Class-name collisions across sections.** The Stellina character rig (Design for Learning) first used `.stel-layer` — which the Stellina immersive SCENE already owns for its 13 injected parallax layers. My CSS bled onto the scene and the scene's bled onto the rig (could break both). Renamed the rig to `.rig-*`. Before reusing a `.stel-*` (or any generic) class, grep it. Also note: **Elena edits `index.html` directly sometimes** — you'll see "file modified on disk" notices; re-read before edits that depend on surrounding markup.
...
- **THE DEPLOY** is the big one — all sections are built + local at v=64; nothing live yet.
- Sticker-sheet **peel animation** (GSAP Flip) — decided-but-deferred polish flourish; the sheet works great without it.
- Stellina character rig: optional whole-body breathe, and the head-sway pivot may want tuning.
...
**Waiting on Elena for content (pre-CD-send, 2026-08-05):**
- **About** intro/overview copy (right column still placeholder)
- Re-exported **hero videos** (currently baked on the old cream bg)
- **Presentation** deck overviews in her voice + any AI-deck "See the presentation" external link
- **Attribution footer legend** — finalise wording (currently a `NOTE FOR ELENA` draft) + confirm it doesn't crowd the meadow on her browser
- **Design for Learning:** ✅ built (both blurbs are hers, in DOM). Tune-ups pending her eyeball: blink feel / head-sway pivot, velt speed, Stellina size, whether the panel wants full-bleed.
- Achievement stories: ✅ DONE (all 7 in `main.js`)
- Social handles (email set: `elenamaria.parr@gmail.com`; socials pending a third-party aggregator)
- **THE DEPLOY** — everything above is local at v=64; a `git push` (real remote exists) or the manual 5-file upload is the only thing between here and a sendable link.
